...
parser.add_argument(
    '-w', '--window', type=float, default=2048, metavar='DURATION',
...
def update_plot(frame):
    ...
    while True:
        ...
        sum_filtered = np.sum(filtered_signal)
        if sum_filtered > amplitude_threshold:
            print('BOOM!!!')
...
try:
    ...
    if args.device is None:
        args.device = int(input("Please enter the device ID for recording: "))
    if args.samplerate is None:
        device_info = sd.query_devices(args.device, 'input')
        args.samplerate = device_info['default_samplerate']
    ...
    filtered_fftplotdata = np.zeros((length, len(args.channels)))
